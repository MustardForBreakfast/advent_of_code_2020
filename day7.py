...
def _parse_container_contents(rule_line: str) -> t.Tuple[str, str]:
    """
...
def _parse_content_bag_and_count(content_str: str) -> t.Tuple[str, int]:
...
def _parse_contents(contents_str: str) -> t.Tuple[t.Tuple[str, int], ...]:
    """
...
def parse_rule(rule_line: str) -> t.Tuple[str, t.Tuple[t.Tuple[str, int], ...]]:
    """
    'light red bags contain 1 bright white bag, 2 muted yellow bags.' ->
...
def get_content_bag_count(
    target_bag: str, rules: t.Dict[str, t.Dict[str, int]]
) -> int:
    ...
    sum_contents_contents = reduce(
        lambda acc, b_name: acc
        + (get_content_bag_count(b_name, rules) * bag[b_name]),
        bag.keys(),
        0,
    )
...
def can_contain(
    target_bag: str,
    possible_content_bag: str,
    rules: t.Dict[str, t.Dict[str, int]],
) -> bool:
    ...
    return any(
        map(lambda b: can_contain(b, possible_content_bag, rules), contents)
    )
    ...
    return len(
        tuple(
            filter(
                lambda b: b is True,
                map(
                    lambda b: can_contain(b, possible_content_bag, rules),
                    rules.keys(),
                ),
            )
        )
